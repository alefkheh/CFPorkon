...
from .dnsrecord.cfDNSprogram import addAllCloudflareRecords,deleteCloudflareExistingRecord,getCloudflareExistingRecords
from .dnsrecord.readcredentials import read_cf_credentials
from .dnsrecord.resultsToIP import extract_ip_addresses
...
def _prescan_sigint_handler(sig, frame):
    console.log(
        f"[yellow]KeyboardIntessrrupt detected (pre-scan phase) - {START_DT_STR}[/yellow]"
    )
    exit(1)
        ...
        with multiprocessing.Pool(
            ...
            while True:
                try:
                    ...
                    input_file = INTERIM_RESULTS_PATH
                    output_file = 'tempIPlist.txt'
                    progress.log(f"[green]Starting...[/green]")
                    extract_ip_addresses(input_file, output_file)   
                    with open("tempIPlist.txt", "r") as file:
                        ips = [re.search(r"\b(?:\d{1,3}\.){3}\d{1,3}\b", line).group() for line in file]
                    cfcredentials = "cfcredentials.txt"
                    cloudflare_email,cloudflare_api_key,cloudflare_zone_id,cloudflare_subdomain = read_cf_credentials(cfcredentials)
                    progress.log(f"[red]Deleting records...[/red]")
                    existing_records = getCloudflareExistingRecords(cloudflare_email, cloudflare_api_key, cloudflare_zone_id, cloudflare_subdomain)
                    if not existing_records:
                        progress.log(f"[green]No existing records found. Proceeding to add new records.[/green]")
                    else:
                        progress.log(f"[yellow]Existing records found. Deleting...[/yellow]")
                        with concurrent.futures.ThreadPoolExecutor() as executor:
                            futures = []
                            for record in existing_records:
                                futures.append(executor.submit(deleteCloudflareExistingRecord, cloudflare_email, cloudflare_api_key, cloudflare_zone_id, record["id"]))
                            concurrent.futures.wait(futures)
                        progress.log(f"[green]Existing records deleted.[/green]")
                    progress.log(f"[yellow]Adding new records...[/yellow]")
                    addAllCloudflareRecords(cloudflare_email, cloudflare_api_key, cloudflare_zone_id, cloudflare_subdomain, ips)
                    progress.log(f"[green]Adding succeeded.[/green]")
                    os.remove("tempIPlist.txt")
                    break
